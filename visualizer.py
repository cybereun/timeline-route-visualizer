...
CAMERA_MOVEMENTS = {
    ...
    'korea': dict(context_fraction=1.00, minimum_context_km=1000.0, maximum_context_km=1000.0,
                  padding=1.0, minimum_span=0.005, zoom_out_alpha=0.0,
                  zoom_in_alpha=0.0, leg_aware=False, fixed_zoom=True),
}
...
def fetch_tile_img(x: int, y: int, z: int) -> Image.Image:
    ...
    for url in candidate_urls:
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as response:
                if response.headers.get('x-blocked') or 'blocked' in response.headers.get('X-Blocked', ''):
                    continue
                data = response.read()
                if len(data) < 500:
                    continue
    ...
    if _tile_fetch_failures <= 5:
        print(f"Warning: Could not fetch tile for ({x}, {y}, {z})", file=sys.stderr)
...
def parse_timeline(
    ...
    # timeline.db가 존재하고 JSON 파일이 없거나 접근 불가하면 DB에서 초고속 로드
    use_db = False
    if not Path(input_path).is_file() and db_path.is_file():
        use_db = True
    elif str(input_path).endswith('.db') and Path(input_path).is_file():
        use_db = True
        db_path = Path(input_path)

    if use_db:
        print(f"Loading from indexed database: {db_path}...")
        import sqlite3
        conn = sqlite3.connect(db_path)
        c = conn.cursor()
        query = "SELECT time, lat, lng FROM points WHERE lat IS NOT NULL AND lng IS NOT NULL"
        params = []
        if year:
            query += " AND date LIKE ?"
        ...
        for r_time, r_lat, r_lng in rows:
            dt_parsed = _parse_timeline_timestamp(r_time)
            if dt_parsed and r_lat is not None and r_lng is not None:
                raw_points.append({
                    'dt': dt_parsed[0],
                    'lat': float(r_lat),
                    'lon': float(r_lng),
                    'time_zone_missing': dt_parsed[1],
                })
    else:
...
def build_camera_track(
    ...
    if movement_name == 'korea':
        k_min_lat = min([33.0] + lats) if lats else 33.0
        k_max_lat = max([38.6] + lats) if lats else 38.6
        k_min_lon = min([124.6] + lons) if lons else 124.6
        k_max_lon = max([131.0] + lons) if lons else 131.0

        center_lat = (k_min_lat + k_max_lat) / 2.0
        center_lon = (k_min_lon + k_max_lon) / 2.0
        cx, cy = latlon_to_meters(center_lat, center_lon)

        _, y_s = latlon_to_meters(k_min_lat, center_lon)
        _, y_n = latlon_to_meters(k_max_lat, center_lon)
        lat_diff = abs(y_n - y_s)

        span_y = max(lat_diff * 1.35, 800000.0)
        span_x = span_y * aspect

        x_w, _ = latlon_to_meters(center_lat, k_min_lon)
        x_e, _ = latlon_to_meters(center_lat, k_max_lon)
        lon_diff = abs(x_e - x_w)
        if span_x < lon_diff * 1.25:
            span_x = lon_diff * 1.25
            span_y = span_x / max(0.1, aspect)

        return [(cx, cy, span_x, span_y)] * (CAMERA_TRACK_SAMPLES + 1)
...
def existing_file(value: str) -> Path:
    ...
    if path.is_file():
        return path
    temp_dir = Path(r"C:\Temp\timeline")
    if temp_dir.is_dir():
        jsons = list(temp_dir.glob("*.json"))
        if jsons:
            return jsons[0]
    db_path = Path(__file__).parent / "timeline.db"
    if db_path.is_file():
...
def ensure_ffmpeg_available() -> None:
    ...
    if not animation.writers.is_available('ffmpeg'):
        raise FfmpegUnavailableError(
            "ffmpeg is required to create MP4 video. Install ffmpeg or run: pip install imageio-ffmpeg",
        )

    # 한글 폰트 설정 (Windows 맑은 고딕 등)
    try:
        import platform
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--input', '-i', required=False, default="auto", type=existing_file, help="Path to Timeline.json")
...
def _main_inner(argv: Optional[List[str]] = None) -> int:
    ...
    card_height_ratio = 0.16 * (width_px / height_px)
    card_patch = patches.FancyBboxPatch(
        (0.5 - card_width_ratio / 2, 0.97 - card_height_ratio),
    ...
    def update(i: int) -> Tuple[Any, ...]:
        ...
        if args.camera_movement != 'korea' and i % 4 == 0:
