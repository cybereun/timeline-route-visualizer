...
- 📱 **Threads / Instagram Reels 스타일 세로 영상(9:16) 원클릭 제작**
  - 브라우저에서 감각적인 핑크빛 궤적 애니메이션 영상(MP4 또는 WebM)을 바로 제작
...
4. 렌더링 중 브라우저 탭을 열어 두고, 완성된 영상을 인앱 플레이어로 확인한 뒤 **[영상 다운로드]**를 누릅니다. MP4 미지원 브라우저는 WebM으로 저장합니다.
...
└── static/                # 웹 프론트엔드 정적 자산
    ...
    ├── app.js             # Leaflet 지도 제어, 영상 제작, PWA 처리
    ├── video-renderer.js  # 브라우저 Canvas/MediaRecorder 영상 제작
...
- 영상 제작은 브라우저 Canvas와 MediaRecorder로 처리하므로 Vercel에서도 동작합니다. 브라우저가 MP4 녹화를 지원하지 않으면 WebM 파일을 만듭니다. 렌더링 중에는 브라우저 탭을 열어 두세요.

---

## 🔒 개인정보 보호 (Privacy First)

- 타임라인 파일은 브라우저 IndexedDB에 저장되고 영상은 브라우저 안에서 만들어집니다. JSON 파일과 상세 이동 경로는 앱 서버에 업로드되지 않습니다.
- 배경 지도를 표시하고 영상에 넣기 위해 ESRI 지도 타일을 요청합니다. 이 요청으로 지도에 표시되는 지역의 타일 좌표가 ESRI에 전달되지만, 타임라인 파일이나 경로 좌표 목록은 전송되지 않습니다.
